  ...
  <div class="container mx-auto p-6">
    <div class="flex flex-col md:flex-row justify-between items-start md:items-center mb-8 gap-4">
      <div>
        ...
        <p class="text-muted-foreground">Monitoraggio del completamento missioni per utente.</p>
      </div>
    </div>
    ...
    <Card class="w-full shadow-sm">
      <CardHeader class="flex flex-row items-center justify-between space-y-0 pb-7">
        <div class="space-y-1">
          <CardTitle class="text-xl font-semibold">Distribuzione Completamenti</CardTitle>
          <CardDescription>Visualizzazione aggregata dei dati backend</CardDescription>
        </div>
        <BarChart3 class="h-5 w-5 text-muted-foreground" />
      </CardHeader>
      
      <CardContent>
        <div v-if="isLoading" class="flex flex-col items-center justify-center h-[400px] gap-3">
          <Loader2 class="h-8 w-8 animate-spin text-primary" />
          <p class="text-sm text-muted-foreground">Elaborazione dati in corso...</p>
        </div>

        <div v-else-if="error" class="flex flex-col items-center justify-center h-[400px] text-center border border-destructive/20 rounded-lg bg-destructive/10 p-6">
          <AlertCircle class="h-10 w-10 text-destructive mb-2" />
          <p class="text-destructive font-medium">{{ error }}</p>
          <button @click="fetchStats" class="mt-4 text-sm underline hover:opacity-80">Riprova</button>
        </div>

        <div v-else class="unovis-wrapper">
          <VisXYContainer :data="chartData" :height="400" :margin="{ top: 10, right: 10, bottom: 40, left: 40 }">
            <VisStackedBar 
              :x="x" 
              :y="y" 
              :rounded-corners="4"
