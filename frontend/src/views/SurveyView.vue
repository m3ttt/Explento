...
const canProceed = computed(() => {
  if (currentStepData.value.multipleChoice) {
    ...
    return selectedCategories.value.some((id) => currentOptionIds.includes(id));
  }
...
const handleFinish = async () => {
  ...
  const resp = await makeUserAuthenticatedRequest(`/me/preferences`, {
    method: "POST",
    headers: {
...
<template>
  ...
  >
    <h1 class="text-3xl text-foreground font-bold">Questionario Interessi</h1>
    <p class="text-muted-foreground">
      Compila il questionario per ricevere consigli personalizzati sui luoghi da
      visitare
    </p>
    <div class="space-y-2">
      <div
    ...
    <div class="flex gap-3 pt-4">
      ...
      <Button
        ...
        :disabled="!canProceed"
        @click="currentStep === totalSteps ? handleFinish() : currentStep++"
      >
